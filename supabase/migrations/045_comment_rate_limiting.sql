-- ============================================
-- Comment rate limiting via RPC
-- ============================================
-- Rate limits:
-- Comments:
--   5 comments / minute / user
--   30 comments / hour / user
-- Thread spam (same post, same user):
--   3 comments in 60 seconds
-- Mentions:
--   Max 5 unique mentions per comment
--   10 mention notifications / hour
--   30 mention notifications / day
-- Notifications:
--   20 notifications / hour / user (as actor)

BEGIN;

-- ============================================
-- 1. Remove direct insert policy on comments
-- ============================================
DROP POLICY IF EXISTS "insert own comments" ON public.comments;

-- Create a policy that blocks all direct inserts (only RPC can insert)
CREATE POLICY "no direct inserts" ON public.comments
  FOR INSERT
  WITH CHECK (false);

-- ============================================
-- 2. Create the rate-limited comment creation RPC
-- ============================================
...
BEGIN
  ...
  VALUES (p_post_id, v_author_id, p_content)
  ...
  IF v_mention_count > 0 THEN
    ...
    LOOP
      ...
      IF v_mention_notif_day >= 30 THEN
        EXIT; -- Stop processing mentions
      END IF;
      ...
      IF v_mentioned_user_id = v_post_owner THEN
        CONTINUE;
      END IF;

      -- Verify user exists
      IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = v_mentioned_user_id) THEN
        CONTINUE;
      END IF;
...
-- ============================================
-- 3. Drop the old comment notification trigger
--    (notifications are now handled in the RPC)
-- ============================================
