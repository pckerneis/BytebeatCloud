-- Refactor feed functions to return posts_with_meta rows so that
-- changes to the posts_with_meta view automatically flow into feeds.
DROP FUNCTION IF EXISTS public.get_global_feed(
  page integer,
  page_size integer
);

DROP FUNCTION IF EXISTS public.get_personalized_feed(
  viewer_id uuid,
  page integer,
  page_size integer
);
